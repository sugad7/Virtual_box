# TWO PASS ASSEMBLER (PASS 1 - FULL FIXED)
        ...
        # Imperative Statements
            ...
            if len(parts) > 2:
                ...
                if operand.startswith("='"):
                    if not any(l[0] == operand for l in littab):
                        littab.append([operand, None])

                    index = next(i for i, l in enumerate(littab) if l[0] == operand)
                    code_line += f" (L,{index})"
                ...
                # Symbol
                    ...
                    if operand not in symtab:
                        symtab[operand] = None
                    index = list(symtab.keys()).index(operand)
                    code_line += f" (S,{index})"
